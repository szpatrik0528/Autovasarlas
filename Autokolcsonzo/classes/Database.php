...
class Database {
    ...
    public function setKivalasztottAuto($cars_id, $gyarto_es_modell, $evjarat, $szin, $uzemanyag, $valto_tipus, $utasok_szama, $ar) {
        $stmt = $this->db->prepare("UPDATE `car` SET `gyarto_es_modell`= ?,`evjarat`= ?,`szin`= ?,`uzemanyag`= ?,`valto_tipus`= ?,`utasok_szama`= ?,`ar`= ? WHERE cars_id= ?");
        $stmt->bind_param('isssssss', $cars_id, $gyarto_es_modell, $evjarat, $szin, $uzemanyag, $valto_tipus, $utasok_szama, $ar);
        return $stmt->execute();
    }
